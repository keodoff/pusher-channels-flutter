...
public class PusherOptions {
    ...
    public PusherOptions setWsPath(String wsPath) {
        this.wsPath = wsPath;
        return this;
    }
    ...
    public String buildUrl(final String apiKey) {
        // Проверяем путь: если он пустой - ничего не добавляем,
        // если нет - следим, чтобы он начинался с /
        ...
        return String.format(
                "%s://%s:%s%s/app/%s%s", // Добавлен %s для пути
                useTLS ? WSS_SCHEME : WS_SCHEME,
                host,
                useTLS ? wssPort : wsPort,
                path, // Подставляем путь
                apiKey,
                URI_SUFFIX
        );
    }
